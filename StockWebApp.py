...
#     st.set_page_config(page_title='Khuyến nghị giao dịch cổ phiếu', page_icon=None,layout="wide",initial_sidebar_state='auto')
    ...
    with col1:
        ...
        st.markdown('<p style="font-size:25px">Danh sách cổ phiếu có xu hướng tăng</p>',unsafe_allow_html=True)
        st.dataframe(exportList2.assign(hack='').set_index('hack'))
    with col2:
        st.plotly_chart(dandatplot)
        st.markdown('<p style="font-size:25px">Danh sách cổ phiếu khuyến nghị mua hôm nay</p>',unsafe_allow_html=True)
